...
export default function NotificationButton() {
  ...
  return (
    <div className=" flex items-center relative h-full ">
      ...
      <div className="absolute min-w-max p-3 right-0 top-full z-50 translate-y-3 bg-black-400  rounded-lg before:absolute before:w-0 before:h-0 before:border-[10px] before:top-0 before:right-0 before:-translate-y-[calc(100%-5px)] before:border-transparent before:border-b-black-400/55  backdrop:blur-lg shadow-lg shadow-black-100">
          ...
          {notifications.map((notify) => (
            <li key={notify.id} className="p-3">
              {notify.type === NotificationTypes.SALARY_REQUEST &&
                `${notify.name} ask you to paid his salary`}
            </li>
          ))}
        </ul>
      </div>
