...
export default function DashboardSideNav({ links }: Props) {
  const [isOpen] = useSideBarToggleContext();
  return (
    <aside
      className={cn(
        "w-60 shadow-lg  bg-black-400 max-md:absolute max-md:h-full max-md:-left-full max-md:transition-all max-md:duration-300 max-md:invisible",
        {
